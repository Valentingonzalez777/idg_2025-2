...
tabla_gasto <- tabla_gasto[, c("sexo", "edad", "edue", "ing_pc", "gasto_pasteleria_total", "grupo_escolaridad")]
...
modelo_lineal = lm(log_gasto_pasteleria ~ grupo_escolaridad + ing_pc + rango_edad + factor(sexo), data = tabla_gasto)
...
# --- 1. PREPARACIÓN DE VARIABLES CASEN (Creación, Conversión y Limpieza) ---
casen$ing_pc <- casen$ypc
# 1.0. APLICACIÓN DE WINZORIZACIÓN DEL INGRESO
casen$ing_pc <- pmax(casen$ing_pc, q_ing[1]); casen$ing_pc <- pmin(casen$ing_pc, q_ing[2]) 

# CRÍTICO: Conversión de Sexo (1/2 a Hombre/Mujer)
casen$sexo <- factor(as.character(casen$sexo), levels = c(1, 2), labels = c("Hombre", "Mujer")) 

# Creación de Factores Categóricos
casen$grupo_escolaridad <- cut(casen$esc, breaks = c(-Inf, 12, 14, 16, Inf), labels = levels(personas_gs$grupo_escolaridad), right = TRUE)
casen$rango_edad <- cut(casen$edad, breaks = c(0, 29, 44, 64, Inf), labels = levels(tabla_gasto$rango_edad), right = TRUE)

# --- ORDEN CRÍTICO DE LIMPIEZA Y ALINEACIÓN ---

# A. Limpieza de NAs: Se eliminan las filas con datos faltantes que causan NAs en la matriz.
casen <- casen %>%
  filter(!is.na(edad) & !is.na(ing_pc) & !is.na(grupo_escolaridad) & !is.na(sexo))

# B. SOLUCIÓN AL ERROR DE CONTRASTES: Aplicamos droplevels DESPUÉS de limpiar NAs.
# Esto elimina los niveles de factor sin observaciones, permitiendo que model.matrix funcione.
casen$grupo_escolaridad <- droplevels(casen$grupo_escolaridad)
casen$rango_edad <- droplevels(casen$rango_edad)
casen$sexo <- droplevels(casen$sexo)
# -------------------------------------------------------------------------------------

# --- 2. MODELO LOGIT Y CLASIFICACIÓN (VÍA DUMMY NUMÉRICA) ---

# 1. Crear la matriz de diseño (dummies) para la base CASEN
X_logit_casen <- model.matrix(~ sexo + edad + grupo_escolaridad + ing_pc, data = casen)

# 2. Alineación de columnas: Aseguramos que la matriz CASEN tenga las mismas columnas que la EPF
X_logit_casen <- X_logit_casen[, columnas_logit_epf]
...
# 4. Asignación y Clasificación
casen$prob_predicha <- NA
casen$prob_predicha[as.numeric(rownames(X_logit_casen))] <- predicciones_limpias
umbral_optimo <- coords_opt$threshold
casen$clasificacion <- ifelse(casen$prob_predicha >= umbral_optimo, 1, 0)
casen_pred <- casen[casen$clasificacion == 1, ]
casen_pred <- casen_pred %>% filter(!is.na(prob_predicha))


# --- 3. MODELO LINEAL Y PREDICCIÓN DE MONTO (VÍA FACTOR/XLEV) ---

# Alineación final de factores para el modelo Lineal (usando la estructura de la EPF)
casen_pred$grupo_escolaridad <- factor(casen_pred$grupo_escolaridad, levels = levels(tabla_gasto$grupo_escolaridad))
casen_pred$rango_edad <- factor(casen_pred$rango_edad, levels = levels(tabla_gasto$rango_edad))
casen_pred$sexo <- factor(casen_pred$sexo, levels = levels(tabla_gasto$sexo))

# Predicción del monto (Usa xlev para asegurar la alineación)
casen_pred$log_gasto_estimado <- predict(modelo_lineal, newdata = casen_pred, xlev = modelo_lineal$xlevels) 
casen_pred$gasto_estimado <- exp(casen_pred$log_gasto_estimado) - 1

# Winzorización (Control de Outliers en el gasto estimado)
