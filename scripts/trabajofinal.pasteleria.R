...
# --- MODELO LOGIT (Decisión de Gasto) ---
# Primera parte del Two-Part Model: Estima la probabilidad de que una persona incurra en el gasto (incurre_gasto = 1).
...
# Segunda parte del Two-Part Model: Estima el logaritmo del monto gastado, solo para quienes gastan.
modelo_lineal = lm(log_gasto_pasteleria ~ grupo_escolaridad +
                     ing_pc + rango_edad + factor(sexo),
                   data = tabla_gasto)
...
# 1. Curva ROC y Área Bajo la Curva (AUC)
library(pROC)
...
# 2. Cálculo del Umbral Óptimo (Criterio de Youden)
...
#### CASEN: Imputación del Gasto en Pastelería (CÓDIGO FINAL) ####

# Carga base CASEN Región Metropolitana
# NOTA: Asegúrate de que la ruta del archivo sea correcta.
casen <- readRDS("data/casen_base_preprocesado.rds")


# --- 1. RE-PREPARACIÓN DE VARIABLES CASEN (Alineación Inicial de Factores) ---
# Usamos los 'levels' de las bases de entrenamiento (personas_gs y tabla_gasto)

# Crear Grupo de Escolaridad (Necesario para ambos modelos)
casen$grupo_escolaridad <- cut(
  casen$esc,
  breaks = c(-Inf, 12, 14, 16, Inf),
  # Aseguramos que los niveles sean idénticos a los de la EPF:
  labels = levels(personas_gs$grupo_escolaridad), 
  right = TRUE
)

# Crear Rango de Edad (Necesario para Modelo Lineal)
casen$rango_edad <- cut(casen$edad,
                        breaks = c(0, 29, 44, 64, Inf),
                        # Aseguramos que los niveles sean idénticos a los de la EPF:
                        labels = levels(tabla_gasto$rango_edad), 
                        right = TRUE
)

# Ajustar Sexo (Necesario para ambos modelos)
# Forzamos los niveles del factor 'sexo' a ser los mismos que en la EPF
casen$sexo <- factor(as.character(casen$sexo), levels = levels(personas_gs$sexo))

# Ajustar Ingreso (Variable continua)
casen$ing_pc <- casen$ypc

# Limpieza final de NAs en Ingreso
...
# Filtramos la sub-base para el Modelo Lineal
casen_pred <- casen[casen$clasificacion == 1, ]

# --- 3. MODELO LINEAL Y PREDICCIÓN DE MONTO ---
# La alineación de factores se realiza para los factores categóricos
# del modelo Lineal (sexo, grupo_escolaridad, rango_edad).
...
# Predecir el monto en escala logarítmica
casen_pred$log_gasto_estimado <- predict(
  modelo_lineal, 
  newdata = casen_pred, 
  # **CLAVE:** Forzamos a R a usar los niveles de factores aprendidos en el entrenamiento.
  xlev = modelo_lineal$xlevels 
) 

# Volver a escala natural
casen_pred$gasto_estimado <- exp(casen_pred$log_gasto_estimado) - 1


# Winzorización (Control de Outliers)
...
# 1. Gráfico de la densidad REAL (EPF)
...
# 2. Agrega la densidad PREDIICHA (CASEN), ignorando NAs
# Ahora, esta línea debería funcionar y mostrar una distribución, no un solo punto.
